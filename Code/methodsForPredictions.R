pcaDecisionTree <- function(SalePrice, data_train.pca, finalPC, data_test, salePrices_test){
  ...
  #run a decision tree
  library(rpart)
  rpart.model <- rpart(SalePrice ~ .,data = train.data, method = "anova")
  rpart.model
  ...
  rpart.prediction <-round( predict(rpart.model, test.data), digits = 0)
